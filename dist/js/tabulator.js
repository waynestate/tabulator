...
;(function (global, factory) {
	...
	Clipboard.prototype.generateColumnGroupHeaders = function (columns) {
		...
		this.table.columnManager.columns.forEach(function (column) {
			var colData = _this20.processColumnGroup(column);
	...
	Clipboard.prototype.processColumnGroup = function (column) {
		...
		if (subGroups.length) {
			...
			subGroups.forEach(function (subGroup) {
				var subGroupData = _this21.processColumnGroup(subGroup);
	...
	Clipboard.prototype.buildComplexRows = function (config) {
		...
		groups.forEach(function (group) {
			output.push(_this22.processGroupData(group));
		});
	...
	Clipboard.prototype.buildOutput = function (rows, config, params) {
		...
		if (config.rowGroups) {
			rows.forEach(function (row) {
				output = output.concat(_this24.parseRowGroupData(row, columns, config, params, calcs || {}));
			});
		} else {
	...
	Clipboard.prototype.parseRowGroupData = function (group, columns, config, params, calcObj) {
		...
		if (group.subGroups) {
			group.subGroups.forEach(function (subGroup) {
				groupData = groupData.concat(_this25.parseRowGroupData(subGroup, config, params, calcObj[group.key] ? calcObj[group.key].groups || {} : {}));
			});
		} else {
			if (config.columnCalcs) {
		...
		rowSelection: function rowSelection(cell) {
			...
			if (this.table.modExists("selectRow", true)) {
				...
				if (typeof cell.getRow == 'function') {
					...
					checkbox.addEventListener("change", function (e) {
						if (_this26.table.modules.selectRow.selectedRows.length) {
							_this26.table.deselectRow();
						} else {
							_this26.table.selectRow();
						}
					});
	...
	HtmlTableExport.prototype.generateColumnGroupHeaders = function () {
		...
		columns.forEach(function (column) {
			var colData = _this27.processColumnGroup(column);
	...
	HtmlTableExport.prototype.generateHeaderElements = function () {
		...
		rows.forEach(function (row) {
			...
			_this29.mapElementStyles(_this29.table.columnManager.getHeadersElement(), headerEl, ["border-top", "border-left", "border-right", "border-bottom", "background-color", "color", "font-weight", "font-family", "font-size"]);
			...
			row.forEach(function (column) {
				...
				if (_this29.cloneTableStyle) {
					cellEl.style.boxSizing = "border-box";
				}

				_this29.mapElementStyles(column.column.getElement(), cellEl, ["text-align", "border-top", "border-left", "border-right", "border-bottom", "background-color", "color", "font-weight", "font-family", "font-size"]);
				_this29.mapElementStyles(column.column.contentElement, cellEl, ["padding-top", "padding-left", "padding-right", "padding-bottom"]);

				if (column.column.visible) {
					_this29.mapElementStyles(column.column.getElement(), cellEl, ["width"]);
				} else {
				...
				if (column.column.parent) {
					_this29.mapElementStyles(column.column.parent.groupElement, cellEl, ["border-top"]);
				}
	...
	HtmlTableExport.prototype.generateBodyElements = function (visible) {
		...
		this.table.columnManager.columnsByIndex.forEach(function (column) {
			if (_this30.columnVisCheck(column)) {
				columns.push(column);
			}
		});
		...
		rows = rows.filter(function (row) {
			switch (row.type) {
				case "group":
					return _this30.config.rowGroups !== false;
					break;

				case "calc":
					return _this30.config.columnCalcs !== false;
					break;
			}
		...
		rows.forEach(function (row, i) {
			...
			switch (row.type) {
				case "group":
					...
					_this30.mapElementStyles(firstGroup, rowEl, ["border-top", "border-left", "border-right", "border-bottom", "color", "font-weight", "font-family", "font-size", "background-color"]);
					_this30.mapElementStyles(firstGroup, cellEl, ["padding-top", "padding-left", "padding-right", "padding-bottom"]);
					rowEl.appendChild(cellEl);
					break;
				...
				case "row":
					columns.forEach(function (column) {
						...
						if (_this30.table.modExists("format")) {
							value = _this30.table.modules.format.formatValue(cellWrapper);
						} else {
							switch (typeof value === 'undefined' ? 'undefined' : _typeof(value)) {
								case "object":
									value = JSON.stringify(value);
						...
						if (firstCell) {
							_this30.mapElementStyles(firstCell, cellEl, ["padding-top", "padding-left", "padding-right", "padding-bottom", "border-top", "border-left", "border-right", "border-bottom", "color", "font-weight", "font-family", "font-size", "text-align"]);
						}
					...
					_this30.mapElementStyles(styleRow, rowEl, ["border-top", "border-left", "border-right", "border-bottom", "color", "font-weight", "font-family", "font-size", "background-color"]);
					break;
			}
	...
	Page.prototype.generatePageSizeSelectList = function () {
		...
		if (this.pageSizeSelect) {
			...
			while (this.pageSizeSelect.firstChild) {
				...
				_this32.pageSizeSelect.appendChild(itemEl);
			});
	...
	Page.prototype.setPageToRow = function (row) {
		...
		return new Promise(function (resolve, reject) {
			...
			var rows = _this34.table.rowManager.getDisplayRows(_this34.displayIndex - 1);
			var index = rows.indexOf(row);

			if (index > -1) {
				var page = Math.ceil((index + 1) / _this34.size);

				_this34.setPage(page).then(function () {
					resolve();
				}).catch(function () {
					reject();
				});
	...
	Page.prototype.nextPage = function () {
		...
		return new Promise(function (resolve, reject) {
			if (_this36.page < _this36.max) {
				...
				if (!_this36.progressiveLoad) {
					console.warn("Pagination Error - Next page would be greater than maximum page of " + _this36.max + ":", _this36.max + 1);
				}
				reject();
			}
		});
	...
	Page.prototype.trigger = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			switch (_this37.mode) {
				case "local":
					left = _this37.table.rowManager.scrollLeft;

					_this37.table.rowManager.refreshActiveData("page");
					_this37.table.rowManager.scrollHorizontal(left);

					_this37.table.options.pageLoaded.call(_this37.table, _this37.getPage());
					resolve();
					break;
				...
				case "progressive_scroll":
					_this37.table.modules.ajax.blockActiveRequest();
					_this37._getRemotePage().then(function () {
						resolve();
					}).catch(function () {
						reject();
					});
				...
				default:
					console.warn("Pagination Error - no such pagination mode:", _this37.mode);
					reject();
			}
		});
	};
	...
	Page.prototype._getRemotePage = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			pageParams[_this38.paginationDataSentNames.page] = self.page;

			//set page size if defined
			if (_this38.size) {
				pageParams[_this38.paginationDataSentNames.size] = _this38.size;
			}

			//set sort data if defined
			if (_this38.table.options.ajaxSorting && _this38.table.modExists("sort")) {
				var sorters = self.table.modules.sort.getSort();
				...
				pageParams[_this38.paginationDataSentNames.sorters] = sorters;
			}

			//set filter data if defined
			if (_this38.table.options.ajaxFiltering && _this38.table.modExists("filter")) {
				var filters = self.table.modules.filter.getFilters(true, true);
				pageParams[_this38.paginationDataSentNames.filters] = filters;
			}

			self.table.modules.ajax.setParams(pageParams);

			self.table.modules.ajax.sendRequest(_this38.progressiveLoad).then(function (data) {
